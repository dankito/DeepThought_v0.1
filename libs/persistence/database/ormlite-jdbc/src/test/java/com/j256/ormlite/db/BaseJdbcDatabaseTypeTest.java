...
public abstract class BaseJdbcDatabaseTypeTest extends BaseJdbcTest {
    ...
    PropertyConfig fieldType = tableInfo.getIdProperty();
		...
		if (!sb.toString().contains("PRIMARY KEY")) {
			databaseType.addPrimaryKeySql(new PropertyConfig[] { fieldType }, additionalArgs, null, moreStmts, queriesAfter);
	...
	public void testGeneratedIdSequence() throws Exception {
		...
		List<String> queriesAfter = new ArrayList<String>();
		databaseType.appendColumnArg(null, sb, tableInfo.getPropertyConfigs()[0], additionalArgs, statementsBefore,
