...
public class JdbcDatabaseConnectionTest extends BaseJdbcTest {
	...
	public void testUpdateReleaseConnection() throws Exception {
		...
		jdc.update(statement, new Object[0], new PropertyConfig[0]);
		jdc.close();
	...
	public void testInsertReleaseConnection() throws Exception {
		...
		jdc.insert(statement, new Object[0], new PropertyConfig[0], keyHolder);
		jdc.close();
	...
	public void testQueryForOneReleaseConnection() throws Exception {
		...
		jdc.queryForOne(statement, new Object[0], new PropertyConfig[0], rowMapper, null);
		jdc.close();
	...
	public void testQueryKeyHolderNoKeys() throws Exception {
		...
		try {
			...
			keyHolder.addKey(0L);
			replay(keyHolder);
			databaseConnection.insert("insert into foo (id) values (2)", new Object[0], new PropertyConfig[0], keyHolder);
			verify(keyHolder);
		} finally {
			connectionSource.releaseConnection(databaseConnection);
		}
	...
	public void testIdColumnInteger() throws Exception {
		...
		try {
			...
			keyHolder.addKey(1L);
			replay(keyHolder);
			databaseConnection.insert("insert into fooint (stuff) values (2)", new Object[0], new PropertyConfig[0],
					keyHolder);
			verify(keyHolder);
		} finally {
			connectionSource.releaseConnection(databaseConnection);
	...
	public void testIdColumnInvalid() throws Exception {
		...
		try {
			...
			keyHolder.addKey(1L);
			replay(keyHolder);
			databaseConnection.insert("insert into fooint (stuff) values ('zipper')", new Object[0], new PropertyConfig[0],
					keyHolder);
			verify(keyHolder);
		} finally {
			connectionSource.releaseConnection(databaseConnection);
	...
	public void testIdColumnChangedFromStringToNumber() throws Exception {
		...
		try {
			...
			databaseConnection.insert("insert into fooint (id, stuff) values ('12', 'zipper')", new Object[0],
					new PropertyConfig[0], keyHolder);
			verify(keyHolder);
		} finally {
			connectionSource.releaseConnection(databaseConnection);
		}
