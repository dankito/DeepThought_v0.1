...
public class StatementExecutor<T, ID> implements GenericRowMapper<String[]> {
	...
	private PropertyConfig[] ifExistsPropertyConfigs;
	 ...
	 */
	public StatementExecutor(DatabaseType databaseType, EntityConfig<T, ID> entityConfig, Dao<T, ID> dao) {
	...
	public T queryForId(DatabaseConnection databaseConnection, ID id, ObjectCache objectCache) throws SQLException {
		if (mappedQueryForId == null) {
			mappedQueryForId = MappedQueryForId.build(databaseType, entityConfig, null);
		}
	...
	public long queryForLong(DatabaseConnection databaseConnection, String query, String[] arguments)
		...
		try {
			compiledStatement =
					databaseConnection.compileStatement(query, StatementType.SELECT, NO_PROPERTY_CONFIGs,
	 ...
	 */
	public SelectIterator<T, ID> buildIterator(Dao<T, ID> classDao, ConnectionSource connectionSource,
			int resultFlags, ObjectCache objectCache) throws SQLException { // cda: don't implement against implementation but interface
		prepareQueryForAll();
	...
	public RawRowMapper<T> getRawRowMapper() {
		if (rawRowMapper == null) {
			rawRowMapper = new RawRowMapperImpl<T, ID>(entityConfig);
		}
		return rawRowMapper;
	}
	 ...
	 */
	public SelectIterator<T, ID> buildIterator(Dao<T, ID> classDao, ConnectionSource connectionSource,
			PreparedStmt<T> preparedStmt, ObjectCache objectCache, int resultFlags) throws SQLException { // cda: don't implement against implementation but interface
		DatabaseConnection connection = connectionSource.getReadOnlyConnection();
		CompiledStatement compiledStatement = null;
		try {
			compiledStatement = preparedStmt.compile(connection, StatementType.SELECT, resultFlags);
			SelectIterator<T, ID> iterator =
					new SelectIterator<T, ID>(entityConfig.getEntityClass(), classDao, preparedStmt, connectionSource,
	...
	public <UO> GenericRawResults<UO> queryRaw(ConnectionSource connectionSource, String query,
		...
		try {
			...
			compiledStatement = null;
			connection = null;
			return rawResults; // TODO: invoke postLoad
		} finally {
			IOUtils.closeThrowSqlException(compiledStatement, "compiled statement");
			if (connection != null) {
				connectionSource.releaseConnection(connection);
   ...
   * Return a results object associated with an internal iterator is mapped by the user's rowMapper.
   */
  public GenericRawResults<String[]> queryRaw(ConnectionSource connectionSource, PreparedQuery preparedQuery, ObjectCache objectCache) throws SQLException {
    logger.debug("executing raw query for: {}", preparedQuery);

    DatabaseConnection connection = connectionSource.getReadOnlyConnection();
    CompiledStatement compiledStatement = null;
    try {
      compiledStatement = preparedQuery.compile(connection, StatementType.SELECT,
          DatabaseConnection.DEFAULT_RESULT_FLAGS);
      GenericRawResults<String[]> rawResults =
          new RawResultsImpl<String[]>(connectionSource, connection, null, String[].class, compiledStatement,
              this, objectCache);
      compiledStatement = null;
      connection = null;
      return rawResults;
    } catch(SQLException ex) {
      logger.error("Could not query raw " + preparedQuery, ex);
      throw ex;
    }
    finally {
      IOUtils.closeThrowSqlException(compiledStatement, "compiled statement");
	...
	public int updateRaw(DatabaseConnection connection, String statement, String[] arguments) throws SQLException {
		...
		CompiledStatement compiledStatement =
				connection.compileStatement(statement, StatementType.UPDATE, NO_PROPERTY_CONFIGs,
						DatabaseConnection.DEFAULT_RESULT_FLAGS);
		try {
			assignStatementArguments(compiledStatement, arguments);
	...
	public int create(DatabaseConnection databaseConnection, T data, ObjectCache objectCache) throws SQLException {
		if (mappedInsert == null) {
			mappedInsert = MappedCreate.build(databaseType, entityConfig);
		}
	...
	public int update(DatabaseConnection databaseConnection, Object data, ObjectCache objectCache) throws SQLException {
		if (mappedUpdate == null) {
			mappedUpdate = MappedUpdate.build(databaseType, entityConfig);
		}
	...
	public int updateId(DatabaseConnection databaseConnection, T data, ID newId, ObjectCache objectCache)
		...
		if (mappedUpdateId == null) {
			mappedUpdateId = MappedUpdateId.build(databaseType, entityConfig);
		}
	...
	public int refresh(DatabaseConnection databaseConnection, T data, ObjectCache objectCache) throws SQLException {
		if (mappedRefresh == null) {
			mappedRefresh = MappedRefresh.build(databaseType, entityConfig);
		}
	...
	public int delete(DatabaseConnection databaseConnection, T data, ObjectCache objectCache) throws SQLException {
		if (mappedDelete == null) {
			mappedDelete = MappedDelete.build(databaseType, entityConfig);
		}
	...
	public int deleteObjects(DatabaseConnection databaseConnection, Collection<T> datas, ObjectCache objectCache)
		...
		int result =
				MappedDeleteCollection.deleteObjects(databaseType, entityConfig, databaseConnection, datas, objectCache);
		if (dao != null && !localIsInBatchMode.get()) {
			dao.notifyChanges();
		}
		return result;
	...
	public int deleteIds(DatabaseConnection databaseConnection, Collection<ID> ids, ObjectCache objectCache)
			throws SQLException {
		// have to build this on the fly because the collection has variable number of args
		int result = MappedDeleteCollection.deleteIds(databaseType, entityConfig, databaseConnection, ids, objectCache);
		if (dao != null && !localIsInBatchMode.get()) {
			dao.notifyChanges();
		}
		return result;
	...
	/**
	 * Call batch tasks inside of a connectionSource which may, or may not, have been "saved".
	 */
		...
		try {
			if (connection.isAutoCommitSupported()) {
				...
				if (autoCommitAtStart) {
					...
					logger.debug("disabled auto-commit on table {} before batch tasks", entityConfig.getTableName());
				}
			}
			try {
			...
			if (autoCommitAtStart) {
				...
				logger.debug("re-enabled auto-commit on table {} after batch tasks", entityConfig.getTableName());
			}
		}
	}
	...
	public boolean ifExists(DatabaseConnection connection, ID id) throws SQLException {
		if (ifExistsQuery == null) {
			QueryBuilder<T, ID> qb = new QueryBuilder<T, ID>(databaseType, entityConfig, dao);
			qb.selectRaw("COUNT(*)");
			/*
			 ...
			 */
			qb.where().eq(entityConfig.getIdProperty().getColumnName(), new SelectArg());
			ifExistsQuery = qb.prepareStatementString();
			ifExistsPropertyConfigs = new PropertyConfig[] { entityConfig.getIdProperty() };
		}
		long count = connection.queryForLong(ifExistsQuery, new Object[] { id }, ifExistsPropertyConfigs);
	...
	private void prepareQueryForAll() throws SQLException {
      ...
      if(entityConfig.getInheritance() != InheritanceType.JOINED)
  			preparedQueryForAll = new QueryBuilder<T, ID>(databaseType, entityConfig, dao).prepare();
      else {
...
//          if(joiningEntityQueryBuilder == null)
//            joiningEntityQueryBuilder = new QueryBuilder(databaseType, parentEntityConfig, parentEntityConfig.getDao() != null ? parentEntityConfig.getDao() : dao);
//          else {
//            QueryBuilder currentQueryBuilder = new QueryBuilder(databaseType, parentEntityConfig, parentEntityConfig.getDao() != null ? parentEntityConfig.getDao() : dao);
//            joiningEntityQueryBuilder.join(idColumnName, idColumnName, currentQueryBuilder, QueryBuilder.JoinType.LEFT, QueryBuilder.JoinWhereOperation.AND);
////            joiningEntityQueryBuilder = currentQueryBuilder; // TODO: is this correct?
//          }
//        }

        joiningEntityQueryBuilder = new QueryBuilder(databaseType, entityConfig, entityConfig.getDao() != null ? entityConfig.getDao() : dao);
        EntityConfig parentEntity = entityConfig.getParentEntityConfig();

        while(parentEntity != null) {
          QueryBuilder parentQueryBuilder = new QueryBuilder(databaseType, parentEntity, parentEntity.getDao() != null ? parentEntity.getDao() : dao);
  ...
  protected void recursivelyAddChildEntitiesJoins(EntityConfig<T, ID> entityConfig, QueryBuilder topLevelQueryBuilder, String idColumnName) throws SQLException {
    for(EntityConfig childEntityConfig : entityConfig.getChildEntityConfigs()) {
      QueryBuilder currentQueryBuilder = new QueryBuilder(databaseType, childEntityConfig, childEntityConfig.getDao() != null ? childEntityConfig.getDao() : dao);
      topLevelQueryBuilder.join(idColumnName, idColumnName, currentQueryBuilder, QueryBuilder.JoinType.LEFT, QueryBuilder.JoinWhereOperation.AND);
//          topLevelQueryBuilder = currentQueryBuilder;

      recursivelyAddChildEntitiesJoins(childEntityConfig, topLevelQueryBuilder, idColumnName);
    }
  }
