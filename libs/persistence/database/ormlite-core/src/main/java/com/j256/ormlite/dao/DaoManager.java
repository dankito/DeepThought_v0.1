 ...
 *
 * <p>
 *   cda: Made methods and fields non so DaoManager is subclassable and methods can be overwritten to implement custom behaviour
 * </p>
 * 
 * @author graywatson
 */
public class DaoManager {
  ...
  protected Logger logger = LoggerFactory.getLogger(DaoManager.class);

	protected Map<Class<?>, DatabaseTableConfig<?>> configMap = null;
  protected Map<ClassConnectionSource, Dao<?, ?>> classMap = null;
	protected Map<TableConfigConnectionSource, Dao<?, ?>> tableConfigMap = null;
	 ...
	 */
	public synchronized <D extends Dao<T, ?>, T> D createDao(ConnectionSource connectionSource, Class<T> clazz)
			throws SQLException {
		...
		if (databaseTable == null || databaseTable.daoClass() == Void.class
			...
			Dao<T, ?> daoTmp;
			if (config == null) {
				daoTmp = createDaoInstance(connectionSource, clazz); // cda: to make Dao instance create customizable
			} else {
				daoTmp = createDaoInstance(connectionSource, config); // cda: to make Dao instance create customizable
			}
			dao = daoTmp;
  ...
  // cda: to make Dao instance create customizable
  protected <T> Dao<T, ?> createDaoInstance(ConnectionSource connectionSource, Class<T> clazz) throws SQLException {
    return BaseDaoImpl.createDao(connectionSource, clazz);
  }

  // cda: to make Dao instance create customizable
  protected <T> Dao<T, ?> createDaoInstance(ConnectionSource connectionSource, DatabaseTableConfig<T> config) throws SQLException {
    return BaseDaoImpl.createDao(connectionSource, config);
  }

	/**
	 * Helper method to lookup a DAO if it has already been associated with the class. Otherwise this returns null.
	 */
	public synchronized <D extends Dao<T, ?>, T> D lookupDao(ConnectionSource connectionSource, Class<T> clazz) {
		if (connectionSource == null) {
			throw new IllegalArgumentException("connectionSource argument cannot be null");
		}
		ClassConnectionSource key = new ClassConnectionSource(connectionSource, clazz);
	 ...
	 */
	public synchronized <D extends Dao<T, ?>, T> D createDao(ConnectionSource connectionSource,
	 ...
	 * null.
	 */
	public synchronized <D extends Dao<T, ?>, T> D lookupDao(ConnectionSource connectionSource,
	 ...
	 * </p>
	 */
	public synchronized void registerDao(ConnectionSource connectionSource, Dao<?, ?> dao) {
		if (connectionSource == null) {
			throw new IllegalArgumentException("connectionSource argument cannot be null");
		}

    if(lookupDao(connectionSource, dao.getDataClass()) == null)
	 ...
	 * configuration.
	 */
	public synchronized void unregisterDao(ConnectionSource connectionSource, Dao<?, ?> dao) {
		if (connectionSource == null) {
			throw new IllegalArgumentException("connectionSource argument cannot be null");
		}
	 ...
	 */
	public synchronized void registerDaoWithTableConfig(ConnectionSource connectionSource, Dao<?, ?> dao) {
		if (connectionSource == null) {
			throw new IllegalArgumentException("connectionSource argument cannot be null");
		}
	 ...
	 */
	public synchronized void clearDaoCache() {
		if (classMap != null) {
			classMap.clear();
			classMap = null;
		}
	 ...
	 */
	public synchronized void addCachedDatabaseConfigs(Collection<DatabaseTableConfig<?>> configs) {
	...
	protected void addDaoToClassMap(ClassConnectionSource key, Dao<?, ?> dao) {
		if (classMap == null) {
	...
	protected void removeDaoToClassMap(ClassConnectionSource key, Dao<?, ?> dao) {
		if (classMap != null) {
			classMap.remove(key);
		}
	}

	protected void addDaoToTableMap(TableConfigConnectionSource key, Dao<?, ?> dao) {
		if (tableConfigMap == null) {
	...
	protected <T> Dao<?, ?> lookupDao(ClassConnectionSource key) {
		if (classMap == null) {
			classMap = new HashMap<ClassConnectionSource, Dao<?, ?>>();
		}
	...
	protected <T> Dao<?, ?> lookupDao(TableConfigConnectionSource key) {
		if (tableConfigMap == null) {
			tableConfigMap = new HashMap<TableConfigConnectionSource, Dao<?, ?>>();
		}
	...
	protected Constructor<?> findConstructor(Class<?> daoClass, Object[] params) {
	 ...
	 */
	protected <D, T> D createDaoFromConfig(ConnectionSource connectionSource, Class<T> clazz) throws SQLException {
		// no loaded configs
		if (configMap == null) {
			return null;
		}
	...
	protected <D extends Dao<T, ?>, T> D doCreateDao(ConnectionSource connectionSource,
			DatabaseTableConfig<T> tableConfig) throws SQLException {
		...
		if (databaseTable == null || databaseTable.daoClass() == Void.class
				|| databaseTable.daoClass() == BaseDaoImpl.class) {
			Dao<T, ?> daoTmp = createDaoInstance(connectionSource, tableConfig); // cda: to make Dao instance create customizable
			dao = daoTmp;
		} else {
			Class<?> daoClass = databaseTable.daoClass();
	 ...
	 */
	protected class ClassConnectionSource {
      ...
      // cda: sometimes equals() fails because connection is wrapped in a WrappedConnectionSource
      // as i don't know what WrappedConnectionSources are good for and for me it's important that Dao registry in DaoManager works perfectly, i try to compare inner cs of a wrapped cs
	 ...
	 */
	protected class TableConfigConnectionSource {
		ConnectionSource connectionSource;
