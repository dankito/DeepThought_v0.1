...
public class RuntimeExceptionDao<T, ID> implements CloseableIterable<T> {
	...
	public static <T, ID> RuntimeExceptionDao<T, ID> createDao(ConnectionSource connectionSource, Class<T> clazz)
		...
		Dao<T, ID> castDao = (Dao<T, ID>) Instances.getDaoManager().createDao(connectionSource, clazz); // cda
	...
	public static <T, ID> RuntimeExceptionDao<T, ID> createDao(ConnectionSource connectionSource,
		...
		@SuppressWarnings("unchecked")
		Dao<T, ID> castDao = (Dao<T, ID>) Instances.getDaoManager().createDao(connectionSource, tableConfig); // cda
		return new RuntimeExceptionDao<T, ID>(castDao);
	}
   ...
   * @see Dao#delete(Object)
   */
  public int delete(Object data, Class clazz) {
    try {
      return dao.delete(data, clazz);
    } catch (SQLException e) {
      logMessage(e, "delete threw exception on: " + data);
      throw new RuntimeException(e);
    }
  ...
  public void forEach(Consumer<? super T> action) {
    try {
      ...
      logMessage(e, "forEach threw exception on: " + action);
      throw new RuntimeException(e);
    }
  }
  ...
  public Spliterator<T> spliterator() {
    try {
      return dao.spliterator();
    } catch (Exception e) {
      logMessage(e, "spliterator() threw exception");
      throw new RuntimeException(e);
    }
  }
  ...
  public GenericRawResults<String[]> queryRaw(PreparedQuery<T> preparedQuery) throws SQLException {
    try {
      return dao.queryRaw(preparedQuery);
    } catch (SQLException e) {
      logMessage(e, "query threw exception on: " + preparedQuery);
      throw new RuntimeException(e);
    }
	 ...
	 */
	public <FT> Collection<FT> getEmptyForeignCollection(String fieldName) {
		try {
  ...
  public EntityConfig<T, ID> getTableInfo() {
    return dao.getEntityConfig();
  }

  public DatabaseTableConfig<T> getTableConfig() {
    return dao.getTableConfig();
  }
