...
public abstract class BaseSqliteDatabaseType extends BaseDatabaseType {
	...
	protected void appendLongType(StringBuilder sb, PropertyConfig propertyConfig, int fieldWidth) {
		 ...
		 */
		if (propertyConfig.getSqlTypeOfFieldConverter() == SqlType.LONG && propertyConfig.isGeneratedId()) {
			sb.append("INTEGER");
		} else {
			sb.append("BIGINT");
		}
	...
	protected void configureGeneratedId(String tableName, StringBuilder sb, PropertyConfig propertyConfig,
		 ...
		 */
		if (propertyConfig.getSqlTypeOfFieldConverter() != SqlType.INTEGER && propertyConfig.getSqlTypeOfFieldConverter() != SqlType.LONG) {
	...
	public FieldConverter getFieldConverter(DataPersister dataPersister, PropertyConfig propertyConfig) {
		...
		switch (dataPersister.getSqlType()) {
			...
			default :
				return super.getFieldConverter(dataPersister, propertyConfig);
		}
	}
