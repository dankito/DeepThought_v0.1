...
public abstract class BaseConnectionSource implements ConnectionSource {
	...
	/**
	 * Returns the connectionSource that has been saved or null if none.
	 */
	...
	/**
	 * Return true if the connectionSource being released is the one that has been saved.
	 */
	protected boolean isSavedConnection(DatabaseConnection connection) {
		...
		if (currentSaved == null) {
			...
			// ignore the release when we have a saved connectionSource
			return true;
		} else {
			return false;
		}
	...
	/**
	 * Save this connectionSource as our special connectionSource to be returned by the {@link #getSavedConnection()} method.
	 * 
	 * @return True if the connectionSource was saved or false if it was already saved.
	 */
	protected boolean saveSpecial(DatabaseConnection connection) throws SQLException {
		// check for a connectionSource already saved
		NestedConnection currentSaved = specialConnection.get();
		if (currentSaved == null) {
	...
	/**
	 * Clear the connectionSource that was previously saved.
	 * 
	 * @return True if the connectionSource argument had been saved.
	 */
	protected boolean clearSpecial(DatabaseConnection connection, Logger logger) {
		...
		if (connection == null) {
			...
			if (currentSaved.decrementAndGet() == 0) {
				// we only clear the connectionSource if nested counter is 0
