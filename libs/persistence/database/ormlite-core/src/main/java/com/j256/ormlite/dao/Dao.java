...
public interface Dao<T, ID> extends CloseableIterable<T> {
  ...
  /**
   * Don't know how to solve it otherwise, but due to Java Generic type erasure i found no other way to delete a object from foreignDao.
   * @param data
   * @param clazz
   * @throws SQLException
   */
  public int delete(Object data, Class clazz) throws SQLException;
	 ...
	 * 
	 * <p>
	 * <b>WARNING:</b> Depending on the database type the underlying connectionSource may never be freed -- even if you go all
  ...
  public GenericRawResults<String[]> queryRaw(PreparedQuery<T> preparedQuery) throws SQLException;
	 ...
	 * the callable. Also, "commit()" is <i>not</i> called on the connectionSource at all. If "auto-commit" is disabled then
	 ...
	 * 
	 * <p>
	 * This allocates a connectionSource for this specific thread that will be used in all other DAO operations. The thread
	 * <i>must</i> call {@link #endThreadConnection(DatabaseConnection)} once it is done with the connectionSource. It is
	 ...
	 * 
	 * <p>
	 * This method is used to free the connectionSource returned by the {@link #startThreadConnection()} above.
	 * </p>
	 * 
	 * @param connection
	...
	/**
	 * Set auto-commit mode to be true or false on the connectionSource returned by the {@link #startThreadConnection()}. This
	...
	/**
	 * Return true if the database connectionSource returned by the {@link #startThreadConnection()} is in auto-commit mode
	 ...
	 * this will commit all changes to the database made from that point up to now on the connectionSource returned by the
	 * {@link #startThreadConnection()}. The changes will be written to the database and discarded. The connectionSource will
	...
	/**
	 * If you have previously set auto-commit to false using {@link #setAutoCommit(DatabaseConnection, boolean)} then
	 * this will roll-back and flush all changes to the database made from that point up to now on the connectionSource
	 * returned by the {@link #startThreadConnection()} . None of those changes will be written to the database and are
	 * discarded. The connectionSource will continue to stay in the current auto-commit mode.
	 * 
	 * <p>
	 * <b>WARNING:</b> Chances are you should be using the {@link #callBatchTasks(Callable)} instead of this method
	 * unless you know what you are doing.
  ...
  // cda: The following methods have been introducted to be able to implement against Dao interface and not BaseDaoImpl implementation
