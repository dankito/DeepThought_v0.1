...
public class QueryBuilder<T, ID> extends StatementBuilder<T, ID> {
	...
	private final PropertyConfig idField;
	private PropertyConfig[] resultPropertyConfigs;
	...
	public QueryBuilder(DatabaseType databaseType, EntityConfig<T, ID> entityConfig, Dao<T, ID> dao) {
		super(databaseType, entityConfig, dao, StatementType.SELECT);
	...
	public QueryBuilder<T, ID> groupBy(String columnName) {
		PropertyConfig propertyConfig = verifyColumnName(columnName);
	...
	@Override
	protected PropertyConfig[] getResultPropertyConfigs() {
		return resultPropertyConfigs;
	}
	...
	private void matchJoinedFieldsByName(JoinInfo joinInfo, String localColumnName, String joinedColumnName,
			QueryBuilder<?, ?> joinedQueryBuilder) throws SQLException {
		joinInfo.localField = entityConfig.getFieldTypeByColumnName(localColumnName);
		if (joinInfo.localField == null) {
			throw new SQLException("Could not find field in " + entityConfig.getEntityClass() + " that has column-name '"
					+ localColumnName + "'");
		}
		joinInfo.remoteField = joinedQueryBuilder.entityConfig.getFieldTypeByColumnName(joinedColumnName);
		if (joinInfo.remoteField == null) {
			throw new SQLException("Could not find field in " + joinedQueryBuilder.entityConfig.getEntityClass()
	...
	private void matchJoinedFields(JoinInfo joinInfo, QueryBuilder<?, ?> joinedQueryBuilder) throws SQLException {
		for (PropertyConfig propertyConfig : entityConfig.getPropertyConfigs()) {
			...
			PropertyConfig foreignIdField = propertyConfig.getForeignIdField();
			if (propertyConfig.isForeign() && foreignIdField.equals(joinedQueryBuilder.entityConfig.getIdProperty())) {
				joinInfo.localField = propertyConfig;
				joinInfo.remoteField = foreignIdField;
				return;
			}
		}
		// if this other field is a foreign field and its foreign-id field is our id
		for (PropertyConfig propertyConfig : joinedQueryBuilder.entityConfig.getPropertyConfigs()) {
			if (propertyConfig.isForeign() && propertyConfig.getForeignIdField().equals(idField)) {
				joinInfo.localField = idField;
				joinInfo.remoteField = propertyConfig;
				return;
			}
		}

		throw new SQLException("Could not find a foreign " + entityConfig.getEntityClass() + " field in "
				+ joinedQueryBuilder.entityConfig.getEntityClass() + " or vice versa");
	}
...
//      List<FieldType> fieldTypes = new ArrayList<>(Arrays.asList(tableInfo.getFieldTypes()));

      if(joinList != null) {
        for (JoinInfo joinInfo : joinList) {
          for (PropertyConfig propertyConfig : joinInfo.queryBuilder.entityConfig.getPropertyConfigs()) {
            if (propertyConfig.isId() == false && propertyConfig.isTypeIsACollection() == false) {
              sb.append(", ");
              databaseType.appendEscapedEntityName(sb, joinInfo.queryBuilder.tableName);
              sb.append(".");
...
//            fieldTypes.add(fieldType);
      ...
      sb.append(" ");
			...
			resultPropertyConfigs = entityConfig.getPropertyConfigs();
//      resultFieldTypes = fieldTypes.toArray(new FieldType[fieldTypes.size()]);
			return;
		}
		...
		List<PropertyConfig> propertyConfigList = new ArrayList<PropertyConfig>(selectList.size() + 1);
		for (ColumnNameOrRawSql select : selectList) {
			...
			PropertyConfig propertyConfig = entityConfig.getFieldTypeByColumnName(select.getColumnName());
			/*
			...
			appendFieldColumnName(sb, propertyConfig, propertyConfigList);
		...
		if (type != StatementType.SELECT_RAW) {
			// we have to add the idProperty even if it isn't in the columnNameSet
			if (!hasId && selectIdColumn) {
				if (!first) {
					sb.append(',');
				}
				appendFieldColumnName(sb, idField, propertyConfigList);
			}

			resultPropertyConfigs = propertyConfigList.toArray(new PropertyConfig[propertyConfigList.size()]);
		}
		sb.append(' ');
	}

	private void appendFieldColumnName(StringBuilder sb, PropertyConfig propertyConfig, List<PropertyConfig> propertyConfigList) {
		appendColumnName(sb, propertyConfig.getColumnName());
	...
	private class JoinInfo {
		...
		PropertyConfig localField;
		PropertyConfig remoteField;
	...
	public static class InternalQueryBuilderWrapper {
		...
		public PropertyConfig[] getResultFieldTypes() {
			return queryBuilder.getResultPropertyConfigs();
		}
	}
