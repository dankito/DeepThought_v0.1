...
public class MappedPreparedQueryTest extends BaseCoreTest {
	...
	public void testMapRow() throws Exception {
		...
		EntityConfig<LocalFoo, Integer> entityConfig =
				new EntityConfig<LocalFoo, Integer>(connectionSource, null, LocalFoo.class);
		MappedPreparedStmt<LocalFoo, Integer> rowMapper =
				new MappedPreparedStmt<LocalFoo, Integer>(entityConfig, null, new PropertyConfig[0], entityConfig.getPropertyConfigs(),
		...
		try {
			stmt =
					conn.compileStatement("select * from " + TABLE_NAME, StatementType.SELECT, new PropertyConfig[0],
	...
	public void testLimit() throws Exception {
		...
		MappedPreparedStmt<LocalFoo, Integer> preparedQuery =
				new MappedPreparedStmt<LocalFoo, Integer>(entityConfig, "select * from " + TABLE_NAME, new PropertyConfig[0],
						entityConfig.getPropertyConfigs(), new ArgumentHolder[0], 1L, StatementType.SELECT);

		checkResults(foos, preparedQuery, 1);
		preparedQuery =
				new MappedPreparedStmt<LocalFoo, Integer>(entityConfig, "select * from " + TABLE_NAME, new PropertyConfig[0],
						entityConfig.getPropertyConfigs(), new ArgumentHolder[0], null, StatementType.SELECT);
	...
	public void testObjectNoConstructor() throws SQLException {
		new MappedPreparedStmt<NoConstructor, Void>(new EntityConfig<NoConstructor, Void>(connectionSource, null,
				NoConstructor.class), null, new PropertyConfig[0], new PropertyConfig[0], new ArgumentHolder[0], null,
