...
public class BaseMappedQueryTest extends BaseCoreStmtTest {
	...
	public void testMappedQuery() throws Exception {
		...
		PropertyConfig[] resultPropertyConfigs =
				new PropertyConfig[] { Instances.getFieldTypeCreator().createFieldType(connectionSource, tableName, field, Foo.class) };
		BaseMappedQuery<Foo, Integer> baseMappedQuery =
				new BaseMappedQuery<Foo, Integer>(baseFooEntityConfig, "select * from " + tableName, new PropertyConfig[0],
            resultPropertyConfigs) {
				};
