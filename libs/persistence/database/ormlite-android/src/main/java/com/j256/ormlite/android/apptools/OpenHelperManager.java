...
/**
 * This helps organize and access database connections to optimize connectionSource sharing. There are several schemes to
 * manage the database connections in an Android app, but as an app gets more complicated, there are many potential
 * places where database locks can occur. This class allows database connectionSource sharing between multiple threads in a
 * single app.
 * 
 * This gets injected or called with the {@link OrmLiteSqliteOpenHelper} class that is used to manage the database
 * connectionSource. The helper instance will be kept in a static field and only released once its internal usage count goes
 * to 0.
 * 
 * The {@link SQLiteOpenHelper} and database classes maintain one connectionSource under the hood, and prevent locks in the
 * java code. Creating multiple connections can potentially be a source of trouble. This class shares the same
 * connectionSource instance between multiple clients, which will allow multiple activities and services to run at the same
 * time.
 * 
...
public class OpenHelperManager {
	...
	private static <T extends OrmLiteSqliteOpenHelper> T loadHelper(Context context, Class<T> openHelperClass) {
		if (helper == null) {
			 ...
			 * ConnectionSource is tied to a particular database connectionSource. What Filipe was seeing was that when all of
			 * his views we closed (onDestroy), but his application WAS NOT FULLY KILLED, the first View.onCreate()
			 * method would open a new connectionSource to the database. Fine. But because he application was still in memory,
			 ...
			 * ConnectionSource objects and therefore the old database connectionSource. Using those cached collections would
			 ...
			 * connections to the database or re-created the DAOs on a different connectionSource somehow.
			 */
			 ...
			 */
			Instances.getDaoManager().clearDaoCache(); // cda
			instanceCount = 0;
		}
