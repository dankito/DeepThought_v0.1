...
public class DatabaseTableConfigUtil {
  ...
  private final static Logger log = LoggerFactory.getLogger(DatabaseTableConfigUtil.class);
    ...
    DatabaseTableConfig<T> tableConfig = new DatabaseTableConfig<T>(clazz, tableName, null);

    boolean entityUsesJavaxPersistenceAnnotations = clazz.isAnnotationPresent(Entity.class);
    if(entityUsesJavaxPersistenceAnnotations) {
      configureTableFromJavaxPersistenceAnnotations(clazz, tableConfig);
    }

    List<DatabaseFieldConfig> fieldConfigs = getClassFieldConfigs(clazz, databaseType, tableConfig, entityUsesJavaxPersistenceAnnotations);
  ...
  protected static <T> List<DatabaseFieldConfig> getClassFieldConfigs(Class<T> clazz, DatabaseType databaseType, DatabaseTableConfig<T> tableConfig, boolean entityUsesJavaxPersistenceAnnotations) throws SQLException {
    List<DatabaseFieldConfig> fieldConfigs = new ArrayList<DatabaseFieldConfig>();

    for (Class<?> classWalk = clazz; classWalk != null; classWalk = classWalk.getSuperclass()) {
      if(entityUsesJavaxPersistenceAnnotations && JavaxPersistenceImpl.classIsEntityOrMappedSuperclass(clazz) == false)
        break;

      for (Field field : classWalk.getDeclaredFields()) {
        if(field.isAnnotationPresent(Transient.class) == false) { // TODO: also check get-Method
          DatabaseFieldConfig config = configFromField(databaseType, tableConfig.getTableName(), field);
  ...
  protected static <T> void configureTableFromJavaxPersistenceAnnotations(Class<T> entityClass, DatabaseTableConfig<T> tableConfig) throws SQLException {
    Entity entityAnnotation = entityClass.getAnnotation(Entity.class);
    if(entityAnnotation.name().isEmpty() == false)
      tableConfig.setTableName(entityAnnotation.name());

    if(entityClass.isAnnotationPresent(Table.class)) {
      Table tableAnnotation = entityClass.getAnnotation(Table.class);
      if(tableAnnotation.name().isEmpty() == false)
        tableConfig.setTableName(tableAnnotation.name());
      // TODO:
      log.warn("All other settings of @Table annotation then name() are not implemented yet.");
    }

    tableConfig.setInheritanceHierarchy(JavaxPersistenceImpl.getInheritanceHierarchyForClass(entityClass));

    // TODO: implement external callbacks: @EntityListeners, @ExcludeSuperclassListeners, @ExcludeDefaultListeners, see http://www.objectdb.com/java/jpa/persistence/event
  }
