package net.dankito.deepthought.controls.registration;
...
public class FoundRegistrationServerListCell extends ListCell<HostInfo> {
  ...
  protected void updateItem(HostInfo item, boolean empty) {
    ...
    if(empty || item == null)
      ...
      String logoPath = net.dankito.deepthought.util.IconManager.getInstance().getLogoForOperatingSystem(item.getPlatform(), item.getOsVersion(), item.getPlatformArchitecture());
      if(logoPath != null)
